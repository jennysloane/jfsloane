...
# CAN MAYBE DELETE
# something about the API call kept breaking the format of my blog post, so I've saved the data to a csv and loaded it in here instead. Super hacky way to do it but now I don't have to call the API every time also. 

#write_csv(houston, "houston_data.csv")
# houston <- read_csv("houston_data.csv")
```
...
houston2 <- city_function(api_key, my_city="Houston", my_state="TX", my_country="US", start_time = 1606456800, end_time = 1676181600)
houston2_clean <- clean_data_function(houston2) %>%
  mutate(city = "houston")

sydney <- city_function(api_key, my_city="Sydney", my_state="NSW", my_country="AUS", start_time = 1606456800, end_time = 1676181600)
sydney_clean <- clean_data_function(sydney) %>%
  mutate(city = "sydney")

syracuse <- city_function(api_key, my_city="Syracuse", my_state="NY", my_country="US", start_time = 1606456800, end_time = 1676181600)
syracuse_clean <- clean_data_function(syracuse) %>%
  mutate(city = "syracuse")

baltimore <- city_function(api_key, my_city="Baltimore", my_state="MD", my_country="US", start_time = 1606456800, end_time = 1676181600)
...
## Allergies 
    ...
    - This is unfortunate for me because according to my allergy scratch test in 2016, I am **very** allergic (like highest score on the test) to maple and ash and it looks like my allergies may get even worse in March `r emo::ji("upset")`
